...
```html
1. ls -al ~/.ssh 查看是否存在密匙
2. ssh-keygen -t rsa -C ''xxx内容任意，没必要必须邮箱''  添加一个ssh密匙，三下回车，本地管理员目录会出现一个.ssh文件夹，id_rsa是私钥，不能泄露出去，id_rsa.pub是公钥，可以放心地告诉任何人，私钥配置本地，公钥配置远端仓库
3. 验证是否配置秘钥成功 ssh -T git@github.com
4. git remote -v  查看origin
5. git remote rm origin
4. 关联仓库git remote add origin git地址 （ssh）
```
